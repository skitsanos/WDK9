...
namespace WDK.Data.Utils
{
    ...
    public class CollectionPager : Control, IPostBackEventHandler
    {
        ...
        protected bool _IgnoreQueryString;
        ...
        {
            _PagedDataSet = new PagedDataSource {PageSize = 20, CurrentPageIndex = 0, AllowPaging = true};
        }
        ...
        public int CurrentPage
        {
            get
            {
                return _CurrentPage > MaxPages ? MaxPages : _CurrentPage;
            }
        }

        public int RecordStart
        {
            get { return Context == null ? 0 : ((CurrentPage - 1)*_PagedDataSet.PageSize) + 1; }
        }
        ...
        public int TotalRecords
        {
            get
            {
                ...
                if (_PagedDataSet.DataSourceCount > MaxPages*PageSize)
                {
                    return MaxPages*PageSize;
                }
        ...
        {
            set { _PagedDataSet.DataSource = value; }
            get { return _PagedDataSet.DataSource; }
        }
        ...
        public string PageNumbersSeparator
        {
            ...
            set {
                _PageNumbersSeparator = value.Trim() == "" ? "&nbsp;" : value.Trim();
            }
        }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            {
                Control BoundControl = Page.FindControl(_BindToControl);
        ...
        protected virtual void BuildControl()
        {
            ...
            var BackNextNavBack = string.Empty;
            var BackNextNavSeparator = String.Empty;
            var BackNextNavNext= String.Empty;
            ...
            if (ShowPageNumbers)
            {
                ...
                var pages = BuildPageNumbers(CurrentPage, PageCount);
        ...
        protected virtual void BindToQueryString()
        {
            ...
            {
                if (IsNumeric(Context.Request.QueryString[QueryStringKey]))
                {
        ...
        protected virtual string BuildPageNumbers(int currentPage, int pageCount)
        {
            ...
            //End Page            
            var EndPage = pageCount > MaxPages ? MaxPages : pageCount;

            //Adjust for slider
            if (UseSlider)
            {
                var Half = (int) Math.Floor((double) (SliderSize - 1)/2);
                //8: Half = 3
                //7: Half = 3
            ...
            for (var i = StartPage; i <= EndPage; i++)
            {
                ...
                if (Context == null)
                {
                    start = ((i - 1)*_PagedDataSet.PageSize) + 1;
                    end = i*_PagedDataSet.PageSize;
                    ...
                    if (i == StartPage)
                    {
                        ...
                        {
                            Pages.AppendFormat("<b>{0}</b>", i);
                        }
                        else
                        {
                            Pages.AppendFormat("[<b>{0}-{1}</b>]", start, end);
                        }
                    }
                    else
                    {
                        ...
                        {
                            Pages.AppendFormat("[<a href='#'>{0}-{1}</a>]", start, end);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        end = _PagedDataSet.DataSourceCount;
                    }
                    else if (_PagedDataSet.Count == _PagedDataSet.PageSize)
                    {
                        end = (start - 1) + _PagedDataSet.PageSize;
                    }
                    else
                    {
                        end = (start - 1) + _PagedDataSet.Count;
                    }
        ...
        protected virtual string BuildLinkNavBack(int currentPage)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    LinkNav.AppendFormat("<a href='{0}'>{1}</a>"
                                         ,
                                         UpdateQueryStringItem(Context.Request, QueryStringKey,
                                                               Convert.ToString(currentPage - 1))
                                         , BackText);
                }
            }
        ...
        {
            var ButtonNav = new StringBuilder();
            ...
            else
            {
                if (PagingMode == PagingModeType.PostBack)
                {
                    if (ShowFirstLast)
                    {
                        ButtonNav.AppendFormat(
                            "<input type='button' id=\"{0}\" onclick=\"javascript:{1}\" value=\"{2}\"/>"
                            , UniqueID
                            , Page.ClientScript.GetPostBackEventReference(this, Convert.ToString(1))
                            , FirstText);
                    }
                ...
                else
                {
                    ...
                    {
                        ButtonNav.AppendFormat(
                            "<input type='button' onclick=\"window.location='{0}';\" value=\"{1}\"/>&nbsp;"
                            , UpdateQueryStringItem(Context.Request, QueryStringKey, Convert.ToString(1))
                            , FirstText);
                    }
