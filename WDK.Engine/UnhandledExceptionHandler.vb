...
#Region " EmailReport "
        Private Sub EmailReport()
            ...
            Try
                ...
                Dim ms As New IO.MemoryStream(Text.Encoding.UTF8.GetBytes(XmlReport.OuterXml))
...
#Region " XmlReport "
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function XmlReport() As Xml.XmlDocument
            Dim xmlDoc As New Xml.XmlDocument
            Dim xmlDec As Xml.XmlDeclaration = xmlDoc.CreateXmlDeclaration("1.0", "UTF-8", "yes")
            xmlDoc.AppendChild(xmlDec)

            Dim xmlErrors As Xml.XmlElement = xmlDoc.CreateElement("ErrorReport")
            xmlDoc.AppendChild(xmlErrors)

            Try
                Dim errNum As Integer = 1
                For Each err As Exception In HttpContext.Current.AllErrors
                    Dim xmlDebug As Xml.XmlElement = xmlDoc.CreateElement("Debug")
                    xmlErrors.AppendChild(xmlDebug)

                    Dim xmlApp As Xml.XmlElement = xmlDoc.CreateElement("Application")
                    ...
                    Dim xmlErr As Xml.XmlElement = xmlDoc.CreateElement("Error")
                    xmlDebug.AppendChild(xmlErr)

                    Dim xmlErrData As Xml.XmlCDataSection = xmlDoc.CreateCDataSection(err.ToString)
