namespace WDK.Security
{
	...
	//
	public abstract class SymmectricEncrypt : SymmetricAlgorithm
	{
		...
		new static public SymmectricEncrypt Create() 
		{
			return Create("GOST.GOSTsymm");
		}

		new static public SymmectricEncrypt Create(String algName) 
		{
			return (SymmectricEncrypt) CryptoConfig.CreateFromName(algName);
		}
	...
	//
	public class SymmectricEncryptManaged : SymmectricEncrypt
	{
		public SymmectricEncryptManaged()
		{
			//
		}
		...
		private ICryptoTransform _NewCryptor(byte[] rgbKey, CipherMode mode, byte[] rgbIV, int feedbackSize, byte[,] rgbSBoxes, bool bEncrypt)
		{
			...
			return new SymmectricEncryptTransform(rgbKey, mode, rgbIV, BlockSizeValue, feedbackSize, rgbSBoxes, PaddingValue, bEncrypt);
		}
	...
	//
	public class SymmectricEncryptTransform : ICryptoTransform
	{
		...
		public SymmectricEncryptTransform(
			byte[] rgbKey,
			CipherMode mode,
			byte[] rgbIV,
			int blockSize,
