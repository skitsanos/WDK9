...
#Region " Parse() "
    Public Shared Function Parse(ByVal s As String) As W3CDateTime
        Dim Rfc822DateFormat As String = "^((Mon|Tue|Wed|Thu|Fri|Sat|Sun), *)?(?<day>\d\d?) +" & _
          "(?<month>Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) +" & _
          "(?<year>\d\d(\d\d)?) +" & _
          "(?<hour>\d\d):(?<min>\d\d)(:(?<sec>\d\d))? +" & _
          "(?<ofs>([+\-]?\d\d\d\d)|UT|GMT|EST|EDT|CST|CDT|MST|MDT|PST|PDT)$"

        Dim W3CDateFormat As String = _
          "^(?<year>\d\d\d\d)" & _
          "(-(?<month>\d\d)(-(?<day>\d\d)(T(?<hour>\d\d):(?<min>\d\d)(:(?<sec>\d\d)(?<ms>\.\d+)?)?" & _
      "(?<ofs>(Z|[+\-]\d\d:\d\d)))?)?)?$"

        Dim combinedFormat As String = _
          String.Format("(?<rfc822>{0})|(?<w3c>{1})", Rfc822DateFormat, W3CDateFormat)

        ' try to parse it
        Dim reDate As New Regex(combinedFormat)
        Dim m As Match = reDate.Match(s)
        ...
        Try
            Dim isRfc822 As Boolean = m.Groups("rfc822").Success

            Dim year As Integer = Integer.Parse(m.Groups("year").Value)

            ' handle 2-digit and 3-digit years
            If (year < 1000) Then
                If (year < 50) Then year = year + 2000 Else year = year + 1999
            End If
            ...
            Else
                month = ParseField(m.Groups("month"), 1)
            End If

            Dim day As Integer = ParseField(m.Groups("day"), 1)
            Dim hour As Integer = ParseField(m.Groups("hour"), 0)
            Dim min As Integer = ParseField(m.Groups("min"), 0)
            Dim sec As Integer = ParseField(m.Groups("sec"), 0)
            Dim ms As Integer = 0
            If m.Groups("ms").Success Then
                Math.Round((1000 * Double.Parse(m.Groups("ms").Value)))
            End If

            Dim ofs As TimeSpan = TimeSpan.Zero
            If m.Groups("ofs").Success Then
                If isRfc822 Then
            ...
            Dim UtcDate As New DateTime(year, month, day, hour, min, sec, ms)
